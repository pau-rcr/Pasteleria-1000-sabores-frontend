          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <Card key={order.id} className="p-6">
                ...
                {(order.discountDetails.age50Plus ||
                  order.discountDetails.felices50 ||
                  order.discountDetails.birthday) && (
                  <div className="border-t pt-3 mb-3 space-y-1">
                    ...
                    {order.discountDetails.age50Plus && (
                      <p className="text-xs text-muted-foreground">
                        • 50% descuento mayores de 50: -
                        {formatCurrency(order.discountDetails.amountByAge)}
                      </p>
                    )}
                    {order.discountDetails.felices50 && (
                      <p className="text-xs text-muted-foreground">
                        • FELICES50 (10%): -
                        {formatCurrency(order.discountDetails.amountByCode)}
                      </p>
                    )}
                    {order.discountDetails.birthday && (
                      <p className="text-xs text-muted-foreground">
                        • Cumpleaños Duoc: -
                        {formatCurrency(order.discountDetails.amountBirthday)}
                      </p>
                    )}
                  </div>
                )}
